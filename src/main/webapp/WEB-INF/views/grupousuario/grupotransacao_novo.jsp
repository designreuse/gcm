        ...
        <form:form class="form-signin" modelAttribute="lista" id="FormCadastro" action="${insert}" method="PUT">
            ...
            <div class="navbar">
                <div class="navbar-inner">
                    <div class="container-fluid">
                        <a class="brand" style="font: bold;  margin-left: 33%;">Editar Transações do Grupo</a>
                    </div>
                </div>
            </div>
            ...
            <div class="container-fluid navbar navbar-inner">
                ...
                <table class="table table-striped table-bordered table-hover table-condensed active tabelaPaginada" id="example">
                    ...
                    <tr>
                        <th style="width:100px;">ID Transação</th>
                    ...
                    <tr>
                        <td>${lista.codigoTransacao}</td>
                        <td>${lista.descricaoGrupo}</td>
                        <td>${lista.descricaoTransacao}</td>
                        <td>
                            <c:choose>
                                <c:when test="${lista.pertence == true}">
                                    <input class="pertence" id="${lista.id_Transacao}" grupo="${lista.id_GrupoUsuario}" type="checkbox" checked="true"/>
                                </c:when>
                                <c:when test="${lista.pertence != true}">
                                    <input class="pertence" id="${lista.id_Transacao}" grupo="${lista.id_GrupoUsuario}" type="checkbox"/>
                                </c:when>
                            </c:choose>
                        </td>
                    </tr>
...
<script type='text/javascript'>
    $(document).ready(function(){
        ...
        $('.pertence').click(function(){
            ...
            var  id_transacao = $(this).attr('id');
            var  pertence = $(this).prop( "checked" );

            //alert($(this).prop( "checked" ));

            $.getJSON("${pageContext.request.contextPath}/inserir_grupotransacao/"+id_grupo+"/"+id_transacao+"/"+pertence);
        });
    });
</script>