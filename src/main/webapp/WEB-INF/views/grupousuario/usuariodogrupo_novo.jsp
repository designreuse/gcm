        ...
        <form:form class="form-signin" modelAttribute="lista" id="FormCadastro" action="${insert}" method="PUT">
            ...
            <div class="navbar">
                <div class="navbar-inner">
                    <div class="container-fluid">
                        <a class="brand" style="font: bold;  margin-left: 38%;">Editar Usuários do Grupo</a>
                    </div>
                </div>
            </div>
                        ...
                        <td>
                            <c:choose>
                                <c:when test="${lista.pertence == true}">
                                    <input class="pertence" id="${lista.id_Usuario}" grupo="${lista.id_GrupoUsuario}" type="checkbox" checked="true"/>
                                </c:when>
                                <c:when test="${lista.pertence != true}">
                                    <input class="pertence" id="${lista.id_Usuario}" grupo="${lista.id_GrupoUsuario}" type="checkbox"/>
                                </c:when>
                            </c:choose>
                        </td>
                    </tr>
...
<script type='text/javascript'>
    $(document).ready(function(){
        ...
        $('.pertence').click(function(){
            ...
            $.getJSON("${pageContext.request.contextPath}/inserir_usuariodogrupo/"+id_grupo+"/"+id_usuario+"/"+pertence);
        });
    });
</script>