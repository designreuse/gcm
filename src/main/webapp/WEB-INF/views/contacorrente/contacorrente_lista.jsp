    ...
    <div class="navbar">
        <div class="navbar-inner">
            <div class="container-fluid">
                <a class="brand" style="font: bold; margin-left: 300px;">Lista de Contas Correntes</a>
            </div>
        </div>
    </div>
</div>
    ...
    <div class="navbar-inner" style="overflow:auto; height: 350px;">
        ...
        <table class="table table-striped table-bordered table-hover table-condensed active tabelaPaginada" id="example">
            ...
            <tr>
                ...
                <th style="width:80px;">Agência</th>
                <th>Conta</th>
            </tr>
            </thead>
            <tbody>
            <c:forEach items="${lista}" var="p">
                <tr>
                    <td>
                        <a href="/contacorrente_deleta/${p.id_ContaCorrente}" class="btn btn-mini apagar" title="Deletar">
                            <i class="icon-trash"></i>
                        </a>
                        <a href="/contacorrente_editar/${p.id_ContaCorrente}" class="btn btn-mini" title="Editar">
                            <i class="icon-pencil"></i>
                        </a>
                    </td>
                    <td>${p.banco.numeroBanco}</td>
                    <td>${p.numeroAgencia}</td>
                    <td>${p.numeroContaCorrente}</td>
                </tr>
            </c:forEach>
            </tbody>
        </table>
    ...
    <div class="navbar-inner">
        <div class="row-fluid">
            ...
            <div class="span6" style="text-align: right;" >
                <br>
                <a href="<c:url value="/contacorrente_novo"/>" class="btn btn-lg btn-primary" style="width: 70px;"><span class="icon-plus"></span> Novo</a>
            </div>
        </div>
    </div>
</div>
...
<script>
    jQuery(function($){
        $("#Empresas").change(function(){
            location.href="/contacorrente_filtro/"+ $(this).val();
        });
    });
</script>
